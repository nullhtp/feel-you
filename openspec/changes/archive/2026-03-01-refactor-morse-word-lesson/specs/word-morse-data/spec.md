...
#### Scenario: Patterns are derived from morseAlphabet
- **WHEN** `morseWords` is initialized
- **THEN** every word's pattern SHALL be computed from `morseAlphabet` letter patterns using `buildWordPatterns`, not hardcoded
